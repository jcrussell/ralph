...
1. `bd ready{{.Beads.ExcludeFlags}}` — pick the highest-priority
   unblocked issue you can complete in one iteration. Prefer issues
   whose acceptance criteria are testable.
...
If nothing in `bd ready{{.Beads.ExcludeFlags}}` looks small enough to
finish, prefer filing a smaller bead and closing the larger one with
`--reason="too large; split into <new-id>"` over leaving the loop in
