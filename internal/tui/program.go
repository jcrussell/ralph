...
// identity (version, cwd) the model renders above the panel. caps bounds the
// log pane's scrollback ring (config-driven; pass DefaultLogCaps() when there
// is no config source).
//
...
func New(ios *iostreams.IOStreams, initial loop.Snapshot, hdr Header, caps LogCaps, extra ...tea.ProgramOption) *Program {
	...
	p := tea.NewProgram(newModel(ios, initial, hdr, caps), opts...)
