...
- [`bd`](https://github.com/gastownhall/beads) and the `claude` CLI on `$PATH`.
...
```sh
$ ralph init                    # scaffolds .ralph/ next to .git/ (idempotent)
...
`--dry-run` writes the rendered prompt under `.ralph/state/logs/` without invoking the runner. Drop it for a real iteration; drop `--once` for an autonomous session. See [getting started](docs/guides/getting-started.md) for the full walkthrough.
